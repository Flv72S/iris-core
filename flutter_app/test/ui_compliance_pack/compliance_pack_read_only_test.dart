// Phase 11.7.1 — No append; no store mutation.
...
String _tempPath() =>
    '${Directory.systemTemp.path}/iris_compliance_pack_readonly_test.jsonl';
...
Future<void> _populate(LocalFilePersistenceStore store) async {
  ...
  await store.append(TraceRecord.fromTrace(_trace('f')));
  await store.append(TimeContextRecord(
      sessionId: 'session-1', tick: 1, origin: 'trace'));
}

void main() {
  test('generate does not append to store', () async {
    ...
    CompliancePackGenerator().generate(verified);
