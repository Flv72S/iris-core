...
class MetaGovernanceGateEngine {
  ...
  static MetaGovernanceReport evaluate({
    ...
    if (diffResult.hasMetaGovernanceLibChanges) {
      ...
      if (!context.hasValidDecisionAndRatification && context.hasValidGcp) {
        violations.add(MetaGovernanceViolation(
          ...
          description:
              'Decision and ratification must be valid (authority, quorum, version)',
    ...
    if (diffResult.hasCharterFileChanges && !context.hasCharterChangeRatified) {
      violations.add(MetaGovernanceViolation(
        id: 'H10-G7',
        rule: 'CHARTER_INTEGRITY',
        description:
            'META_GOVERNANCE_CHARTER.md modified without GCP and ratification',
        suggestedFix: 'Propose a GCP for the charter change and obtain ratification',
        severity: MetaGovernanceViolationSeverity.error,
      ));
    }
