...
void main() {
  test('current is always last element of history when non-empty', () {
    final impact = GovernanceImpactReport(
      gcpId: const GCPId('GCP-I'),
    ...
    expect(registry.history().isEmpty, isTrue);
    GovernanceActivationEngine.activate(ratification: ratification, registry: registry);
    final hist = registry.history();
    expect(registry.current(), hist.last);
  });

  test('active version is unique per registry (single current)', () {
    final registry = GovernanceRegistry();
    expect(registry.getActiveGovernanceVersion(), isNull);
    final impact = GovernanceImpactReport(
      gcpId: const GCPId('GCP-U'),
      fromVersion: GovernanceVersion(major: 1, minor: 0, patch: 0),
      toVersion: GovernanceVersion(major: 1, minor: 1, patch: 0),
      impacts: const [],
    );
    ...
    GovernanceActivationEngine.activate(ratification: ratification, registry: registry);
    expect(registry.getActiveGovernanceVersion(), isNotNull);
    expect(registry.current(), isNotNull);
    expect(registry.current(), registry.history().last);
  });

  test('no activation without APPROVED: REJECTED throws', () {
    final impact = GovernanceImpactReport(
      gcpId: const GCPId('GCP-N'),
      fromVersion: GovernanceVersion(major: 1, minor: 0, patch: 0),
      toVersion: GovernanceVersion(major: 1, minor: 1, patch: 0),
      impacts: const [],
    );
